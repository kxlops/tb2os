...
unsigned long duration = 5000; // Duration of one full easing cycle (in and out)
...
void displayFrame(const unsigned char* frame, int frameNumber) {
  ...
  if (debugLogs) {
    ...
    for (int j = 0; j < 64; j++) {
      Serial.print(frames[frameNumber][j], HEX);
      Serial.print(" ");
    }
    Serial.println();
  }
...
unsigned long calculateEasedDelay(unsigned long startMillis, unsigned long currentMillis, unsigned long duration) {
  float progress = (float)(currentMillis - startMillis) / (float)duration;
  float easedProgress = 0.5 * (1 - cos(PI * progress)); // Easing function (cosine ease in-out)
  return baseFrameDelay * (1 - easedProgress);
}
...
void loop() {
  ...
  unsigned long easedDelay = calculateEasedDelay(startMillis, currentMillis, duration);

  for (int i = 0; i < numFrames; i++) {
    displayFrame(frames[i], i);
    delay(easedDelay);

    // Update easing cycle timing
